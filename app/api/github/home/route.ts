...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({ repositories: repositories, userData: userData }, { status: 200, headers: {'Cache-Control': 'public, s-maxage=10800, max-age=10800, stale-while-revalidate=60'} });
  } catch (error) {
    console.error(error);
