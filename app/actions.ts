"use server";
import type { PreviousState, ResponseProps } from "@/lib/definitions";
	...
	try {
		...
		if (hasSuccessKey)
			return {
				...
				statusCode: 429,
				message:
					"You have recently sent an email, please wait a while before sending another.",
			} as ResponseProps;

		const getRemainingAttempts = await rateLimiter.getRemaining(attemptKey);
		if (getRemainingAttempts.remaining === 0)
			return {
				status: "fail",
				statusCode: 429,
