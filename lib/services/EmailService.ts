...
import type { ResponseProps } from "../definitions";
	...
	static async send(formdata: FormData) {
		try {
			...
			if (
				...
				!from ||
				(from && validator.isEmail(from) && from.includes(process.env.DOMAIN!))
			)
				return {
					status: "fail",
					statusCode: 400,
					message: "Email must be provided and follow a valid email format",
				} as ResponseProps;
			...
			if (!name || !validator.isAlpha(name, "en-US", { ignore: " " })) {
				return {
					...
					statusCode: 400,
					message:
						"Name must be provided and cannot contain numbers or special characters",
				} as ResponseProps;
			}
			...
			return {
				...
				statusCode: 500,
				message:
					"An internal server error has occurred while trying to send email",
			};
		}
	}
}
