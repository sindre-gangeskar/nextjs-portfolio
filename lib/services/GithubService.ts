			...
			const repos =
				(await octokit
					.request("GET /users/{username}/repos", {
						per_page: 100,
						username: "sindre-gangeskar",
						type: "owner",
						direction: "asc",
					})
					.then((data) =>
						data.data.filter((repo) => combined.includes(repo.name)),
					)) ?? [];
			const { html_url, avatar_url } = repos[0].owner;
			const featured = formatRepos(repos)?.filter((repo) =>
				featuredRepos.includes(repo.original_name),
			);
			const all = formatRepos(repos)?.filter((repo) =>
				allRepos.includes(repo.original_name),
			);
			return { featured, all, owner: { html_url, avatar_url } };
		},
		["github-data"],
		{ revalidate: revalidateAfter, tags: ["github-data"] },
	);
}
...
function formatRepos(arr: RepoType[]) {
	if (Array.isArray(arr)) {
		const formattedRepos = arr.map((x) => ({
			...
			...(x.name === "nextjs-portfolio"
				? { img: "/images/og-default.jpg" }
				: null),
